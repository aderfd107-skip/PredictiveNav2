...
代码把内部 4×4 协方差中的位置部分放进 pose 的 x/y 位置，把速度部分放进 twist 的 vx/vy 位置。其余无法观测的维度不假装“误差为 0”，而是保留数值 0，同时**只在它们自己的对角线**设置很大的方差 `1e6`，表示“当前不知道它们”。

这里必须区分 variance（方差）与 covariance（协方差）：

```text
对角线：变量自身的不确定性
非对角线：两个变量是否一起变化、相关到什么程度
```

所以 2D tracker 的正确 6×6 形状是：
...
`Pxy/Pyx` 是 Kalman Filter 真正估计出的 x/y 相关性，因此从内部 `Track::covariance` 复制。其余非对角线填 0，意思是“当前没有建模它们之间的相关性”；不能因为 z 或 yaw 不可观测，就填一个巨大的交叉协方差。那会错误地声称它们与 x/y 存在极强相关性，也可能破坏协方差矩阵应有的半正定性质。
