...
**本章必须懂**：节点订阅 topic 后，ROS 2 在有新消息时调用 callback；本项目的 callback 就是“每收到一帧 scan，处理一次”。激光传感器数据使用 `SensorDataQoS`，目的是优先拿到新数据，不为等待旧数据而拖慢感知。

**可选扩展**：executor、callback group 与多线程节点暂时都不需要。当前单节点、单回调的模型足够。
