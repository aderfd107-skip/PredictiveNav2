...
**本章必须懂**：`LaserScan` 消息自己提供 `range_min`、`range_max`，它们是传感器报告的可用范围；`NaN` 和 `Inf` 代表不能直接用于几何计算的读数。启动参数让你不改代码就能调整项目阈值。

**可选扩展**：QoS 的 reliability、durability 等完整组合以后排查通信问题时再学；本章只需知道不同传感器通信设置可能不同。
