...
## 14. 进入动态障碍物模块前的三个关键问题

### 问题一：为什么不能直接用 `lidar_link` 坐标做跨帧跟踪？

`lidar_link` 固定在机器人上，会随机器人一起平移、旋转。同一个静止箱子在不同扫描帧中，相对 LiDAR 的距离和角度都会变化；若直接比较这些坐标，会把“机器人自身运动”误判为“箱子在运动”。

因此每帧激光点都应使用该帧的时间戳和 TF，转换到一个短时间连续的固定参考系：

```text
lidar_link → base_footprint → odom
```

在 `odom` 中，静止障碍物在短时间内保持接近相同的位置，跟踪器才能正确计算速度；动态障碍物的位置变化才主要代表它自身的运动。

### 问题二：为什么首版跟踪在 `odom`，而不是直接在 `map`？

先澄清：`map` 是固定的全局地图参考系，RViz 以 `map` 为 fixed frame 时，灰色地图不会移动。`odom` 也不是静止不变的——机器人在 `odom` 中的位姿会随行驶持续变化；它的关键性质是**连续**，不会因 AMCL 定位修正而突然跳变。
...
某个障碍物从 `odom` 换算到 `map` 的关系是：

```text
p_map = T_map_odom(t) × p_odom
```

AMCL 若在某一刻校正 `T_map_odom(t)`，即使障碍物物理上没有突然移动、其 `p_odom` 仍连续，计算得到的 `p_map` 也可能整体跳变。若跟踪器直接用相邻 `map` 位置差计算速度，就可能把定位校正误认为障碍物的高速运动。

PredictiveNav2 首版只预测未来几秒、服务于使用 `odom` 的局部代价地图和控制器，因此将聚类、跟踪、预测状态放在 `odom` 最简单且稳定。需要在全局地图展示或记录 benchmark 时，再通过 `map → odom` 转换。长时间全局跟踪也可以使用 `map`，但必须额外将已有 track 状态同步处理 AMCL 的坐标修正。

一句话记忆：

```text
map：全局固定，适合地图、目标点、全局路径与实验记录。
odom：短时间连续，适合局部控制、动态跟踪、速度估计与短期预测。
```

### 问题三：DWB 已经能避障，为什么还要预测？

DWB 会利用当前 `/scan` 构建局部代价地图，并以 20 Hz 重新选择速度，所以它能对“已经出现在眼前”的障碍物减速、绕行或停车。

但它没有障碍物 ID、速度或未来轨迹。它主要知道：

```text
此时此刻，障碍物在哪里？
```

例如一个人此刻在机器人右侧、尚未挡住路径，但正快速横穿；当前 costmap 可能仍认为前方可走，DWB 通常要等人真正进入路径后才反应。

PredictiveNav2 要从连续扫描中得到：

```text
位置序列 → 同一目标 ID → 速度与协方差 → 未来 1～3 秒位置
```

再把未来风险交给 MPPI critic。这样控制器可以在碰撞风险真正进入当前 costmap 前，提前减速、等待或选择另一条局部轨迹。

```text
DWB：现在这里有没有障碍？
PredictiveNav2：未来几秒，障碍会不会进入我的候选轨迹？
```
