...
class StaticWorldMarkerPublisher(Node):
    def __init__(self):
        super().__init__("static_world_marker_publisher")
        default_world = (
            get_package_share_directory("predictive_nav_simulation")
            + "/worlds/dynamic_navigation_lab.sdf"
        )
        self.declare_parameter("world_file", default_world)

        qos = QoSProfile(depth=1)
        qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        self.publisher = self.create_publisher(MarkerArray, "static_world_markers", qos)
...
def main():
    rclpy.init()
    node = StaticWorldMarkerPublisher()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
